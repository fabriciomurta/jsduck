...
module JsDuck::Tag
  #  This is just a copypaste of the cfg.rb class because a @pattern of
  # (cfg|config) is not supported at all. :weary:
  class Config < MemberTag
    def initialize
      @pattern = "config"
      @tagname = :config
      @repeatable = true
      @member_type = {
        :title => "Config options (alternative/new)",
    ...
    def to_html(config, cls)
      member_link(config) + " : " + config[:html_type]
    end
  end
end
